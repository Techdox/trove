...
- In anything you care about, pin a specific version (e.g. `0.17.0`) <!-- x-release-please-version --> rather than
...
(`ghcr.io/techdox/trove-server:0.17.0` <!-- x-release-please-version --> instead of `:latest`), then `pull` / `up -d`.
...
```sh
go install github.com/techdox/trove/cmd/trove-server@v0.17.0   # x-release-please-version; or @latest
```

...
func TestBackupUpgradeRestoreRollbackCompatibility(t *testing.T) {
	...
	report := &model.Report{
		Agent: model.ReportAgent{Name: "compat-agent", Platform: model.PlatformKubernetes, Version: "0.15.1"},
		Host:  model.ReportHost{Hostname: "compat-host", Meta: map[string]string{"fixture": "v0.15.1"}},
		Services: []model.ReportService{
			{ExternalID: "parent", Name: "compat-parent", Kind: model.KindDeployment, Image: "example.invalid/parent:1", ImageDigest: "sha256:parent", State: "1/1", Health: model.HealthHealthy},
			{ExternalID: "child", ParentExternalID: "parent", Name: "compat-child", Kind: model.KindPod, Image: "example.invalid/child:1", ImageDigest: "sha256:child", State: "running", Health: model.HealthHealthy},
		},
	}
	if err := source.ApplyReport(ctx, agent.ID, report); err != nil {
		t.Fatalf("apply representative report: %v", err)
	}
	if err := source.RecordImageDigest(ctx, "example.invalid/parent:1", "sha256:latest", time.Now().Add(time.Hour).Unix()); err != nil {
		t.Fatalf("seed image freshness record: %v", err)
	}
	if err := source.SetMeta(ctx, "alert_cursor", "1"); err != nil {
		t.Fatalf("seed alert cursor: %v", err)
	}
	if err := source.SetAlertState(ctx, "svc:compat:health", store.AlertState{Value: "healthy", Notified: true, SentAt: 1}); err != nil {
		t.Fatalf("seed alert state: %v", err)
	}
	if err := source.MarkChannelDelivered(ctx, "compat-delivery", "webhook"); err != nil {
		t.Fatalf("seed channel delivery state: %v", err)
	}
	...
	afterOverwriteAttempt, err := os.ReadFile(backupPath)
	if err != nil {
		t.Fatalf("read backup after overwrite refusal: %v", err)
	}
	if got := sha256.Sum256(afterOverwriteAttempt); got != backupHash {
		t.Fatal("backup contents changed after overwrite refusal")
	}
...
func snapshotStore(t *testing.T, st *store.Store) compatibilitySnapshot {
	...
	snapshot := compatibilitySnapshot{
		...
		channelState:   count("SELECT COUNT(*) FROM alert_channel_deliveries"),
		migrations:     count("SELECT COUNT(*) FROM schema_migrations"),
		serviceParents: count("SELECT COUNT(*) FROM services WHERE parent_id IS NOT NULL"),
		orphanHosts:    count("SELECT COUNT(*) FROM hosts h LEFT JOIN agents a ON a.id = h.agent_id WHERE a.id IS NULL"),
		orphanServices: count("SELECT COUNT(*) FROM services s LEFT JOIN hosts h ON h.id = s.host_id WHERE h.id IS NULL"),
		orphanParents:  count("SELECT COUNT(*) FROM services s LEFT JOIN services p ON p.id = s.parent_id WHERE s.parent_id IS NOT NULL AND p.id IS NULL"),
	}
	if snapshot.orphanHosts != 0 || snapshot.orphanServices != 0 || snapshot.orphanParents != 0 {
		t.Fatalf("orphan relationships detected: %+v", snapshot)
	}
	return snapshot
}
